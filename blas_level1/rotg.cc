...
void cblas_srotg (float *a, float *b, float *c, float *s) {
    call_cuda_kernel(cublasSrotg(b2c_handle, a, b, c, s));
}

void cblas_drotg (double *a, double *b, double *c, double *s) {
    call_cuda_kernel(cublasDrotg(b2c_handle, a, b, c, s));
}
...
void cblas_crotg (float _Complex *a,
    ...
    call_cuda_kernel(cublasCrotg(b2c_handle, (cuComplex *) a, &b_tmp, c, (cuComplex *) s));
}
