...
cat < <(env OBJTRACKER_OPTIONS="blas_libs=$1;only_print_calls" LD_PRELOAD=$LIBOBJTRACKER $2 ${@:3} | awk '/C \[0x[0-9a-f]+\].*fun=\[\w+\] reqsize=\[[0-9a-f]+\] ip_offs=\[[0-9a-f\.]+\]/{print $3,$4,$5}' | sort -u) | tee $fname
