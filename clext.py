...
# Modifications by Princeton Ferro
# 
...
def main():
    ...
    status_pattern = re.compile('#define (CL_\S+)\s+(-[0-9]+)$')

    device_type_pattern = re.compile('#define (CL_DEVICE_TYPE_\S+)\s+(\(\d+\s*<<\s*\d+\)|0x[A-Fa-f]+|\d+)$')

    test_codes = []

    write_lines = []

    device_type_get_string_lines = []
    ...
    write_lines.append(' */\n')
    write_lines.append('static inline const char* clGetErrorString(int errorCode) {\n')
    ...
    for f in os.listdir(args.clheaders):
        ...
        with open(path_to_cl_h) as f:
            for line in f.readlines():
                ...
                m = device_type_pattern.match(line.strip())

                if m:
                    string = m.group(1)
                    code = m.group(2)

                    device_type_get_string_lines.append('\t\tcase {0}: return "{1}";\n'.format(code, string))
            pass
    ...
    write_lines.append(' */\n')
    write_lines.append('static inline int clCheckError(int errorCode) {\n')
